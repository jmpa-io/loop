...
- bin/homelab-loop.sh     — Arch laptop executor loop (do NOT modify while loop is running)
- bin/opencode-loop.sh    — Mac fixer loop (this script — do NOT self-modify)
...
" 2>&1)
  ...
  bash "$REPO_DIR/bin/trim-loop-context.sh" 2>/dev/null || true
